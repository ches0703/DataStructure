...
int main()
{
	...
	printf("1.1. 역순 정렬 배열(100,000 ~ 1) 생성 및 정렬, 정렬 검사 수행\n");
	...
	printf("1.2 역순 정렬된 배열의 크기를 0부터 1,000,000까지 바꾸면서 sort 함수의 실행 시간을 측정하여 관찰지에 기록\n");
	printf("      n        time\t   sort\n");
	for (int i = 0; i < 6;i++) {
		int n;
		switch (i)
		...
		case 0:
			...
			printf("%7d %10.3fs     ", 0, duration/1000);
			is_sorted(list, 0);
			n = 10;
			break;
		case 1:
		...
		if (i != 5){
			for (int j = 1; j < 10; j++) {
				...
				printf("%7d %10.3fs     ", n * j, duration/1000);
				is_sorted(list, n * j);
			}
		}
		else {
			...
			printf("%7d %10.3fs     ", n, duration / 1000);
			is_sorted(list, n);
		}
	}
	...
	printf("1.3. 역순 정렬 배열(100,000 ~ 1) 생성 및 정렬, 이진검색 수행\n");
	...
	// 1.4 역순 정렬 배열(50,000 ~ 0) 생성 및 정렬, 이진검색 수행 후 n = 100,000의 배열과 비교

	printf("1.4. 역순 정렬 배열(50,000 ~ 1) 생성 및 정렬, 이진검색 수행\n");

	printf("1.4.1. 역순 배열 생성 : ");
	reverse_list_make(list, 50000);
	printf("생성 완료\n");

	printf("1.4.2. 배열 정렬 : ");
	sort(list, 50000, &duration);
	printf("정렬 완료\n");
	printf("정렬에 걸린 시간 : %.3fs\n", duration / 1000);

	printf("1.4.3. 정렬 상태 : ");
	is_sorted(list, 50000);
	...
	for (;;) {
		...
		printf("검색 데이터 : %6d\t Counter : %d\n", list[binsearch(list, search_data, 0, 50000 - 1, &search_count)], search_count);
	}
